...
// *_CONFIG / *_FILE knobs documented in .env.example.
const EnvFlagFile = "FEATURE_FLAGS_FILE"
...
//  2. StaticProvider loaded from the YAML file at FEATURE_FLAGS_FILE
//     (when the env var is set and the file exists).
//
// When FEATURE_FLAGS_FILE is unset, the Service still works — the
