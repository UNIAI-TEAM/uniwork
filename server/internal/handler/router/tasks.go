...
// registerTasksSuite mounts Work Management suite routes (membership only).
//
...
func registerTasksSuite(r api, h Routes) {
	r.Group(func(suite api) {
		suite.Post("/workspaces/{workspaceID}/tasks/query", h.QueryTasks, apiOp{
			summary:     "Query tasks",
			description: "Lọc và phân trang công việc (suite parity).",
			tags:        []string{"tasks"},
		...
		suite.Get("/workspaces/{workspaceID}/tasks/grouped", h.GroupedTasks, apiOp{
			...
			description: "Nhóm công việc theo status cho board. Query: group_by, status, limit, offset.",
			tags:        []string{"tasks"},
		...
		suite.Put("/tasks/{taskID}", h.PutTaskSuite, apiOp{
			...
			description: "Sửa công việc với revision / If-Match. Lệch → 422 revision_conflict.",
			tags:        []string{"tasks"},
			sdi:         sdi.PutTaskSDI{},
			sdo:         sdo.TaskSDO{},
			auth:        true,
		...
		suite.Post("/workspaces/{workspaceID}/tasks/batch-update", h.BatchUpdateTasks, apiOp{
			...
			description: "Cập nhật nhiều công việc cùng patch.",
			tags:        []string{"tasks"},
		...
		suite.Post("/workspaces/{workspaceID}/tasks/batch-delete", h.BatchDeleteTasks, apiOp{
			...
			description: "Xóa nhiều công việc.",
			tags:        []string{"tasks"},
		...
		suite.Get("/workspaces/{workspaceID}/my-tasks", h.ListMyTasks, apiOp{
			summary:     "My tasks",
			description: "Công việc của người gọi trong workspace (relation=all|assigned|created|involved).",
			tags:        []string{"tasks"},
		...
		suite.Get("/workspaces/{workspaceID}/tasks/children", h.ListChildrenByParents, apiOp{
			...
			description: "Query parent_ids=id,id — batch children cho Swimlane.",
			tags:        []string{"tasks"},
			sdo:         sdo.TaskListSDO{},
			auth:        true,
		})
		suite.Get("/workspaces/{workspaceID}/tasks/child-progress", h.ChildTaskProgress, apiOp{
			...
			description: "Tổng/done theo từng parent trong workspace.",
			tags:        []string{"tasks"},
		...
		suite.Put("/tasks/{taskID}/parent", h.SetTaskParent, apiOp{
			...
			description: "Gán hoặc gỡ cha. Chu trình → 422 parent_cycle.",
			tags:        []string{"tasks"},
		...
		suite.Post("/tasks/{taskID}/dependencies", h.SetTaskDependency, apiOp{
			...
			description: "Thêm phụ thuộc blocks/blocked_by/related. Chu trình → 422 parent_cycle.",
			tags:        []string{"tasks"},
		...
		suite.Delete("/tasks/{taskID}/dependencies/{dependsOnTaskID}", h.RemoveTaskDependency, apiOp{
			...
			description: "Xóa cạnh phụ thuộc. Query type tùy chọn.",
			tags:        []string{"tasks"},
			sdo:         sdo.StatusSDO{},
			auth:        true,
		})
		...
		suite.Get("/workspaces/{workspaceID}/task-statuses", h.ListTaskStatuses, apiOp{
			summary: "List task statuses", description: "Catalog trạng thái workspace.",
		...
		suite.Post("/workspaces/{workspaceID}/task-statuses", h.CreateTaskStatus, apiOp{
			summary: "Create task status", description: "Thêm trạng thái tùy chỉnh.",
		...
		suite.Patch("/workspaces/{workspaceID}/task-statuses/reorder", h.ReorderTaskStatuses, apiOp{
			summary: "Reorder task statuses", description: "Sắp xếp custom status trong một category.",
		...
		suite.Get("/workspaces/{workspaceID}/task-labels", h.ListTaskLabels, apiOp{
			summary: "List task labels", description: "Danh sách nhãn workspace.",
		...
		suite.Post("/workspaces/{workspaceID}/task-labels", h.CreateTaskLabel, apiOp{
			summary: "Create task label", description: "Tạo nhãn.",
		...
		suite.Delete("/workspaces/{workspaceID}/task-labels/{id}", h.DeleteTaskLabel, apiOp{
			summary: "Delete task label", description: "Xóa nhãn và gỡ khỏi task.",
			tags: []string{"tasks"}, sdo: sdo.StatusSDO{}, auth: true,
		})

		suite.Get("/workspaces/{workspaceID}/task-properties", h.ListTaskProperties, apiOp{
			summary: "List task properties", description: "Định nghĩa thuộc tính tùy chỉnh.",
		...
		suite.Post("/workspaces/{workspaceID}/task-properties", h.CreateTaskProperty, apiOp{
			summary: "Create task property", description: "Tạo thuộc tính tùy chỉnh.",
		...
		suite.Patch("/workspaces/{workspaceID}/task-properties/{id}", h.PatchTaskProperty, apiOp{
			summary: "Update task property", description: "Sửa hoặc archive thuộc tính.",
		...
		suite.Get("/tasks/{taskID}/labels", h.ListTaskLabelsOnTask, apiOp{
			summary: "List labels on task", description: "Nhãn gắn trên một task.",
			tags: []string{"tasks"}, sdo: sdo.TaskLabelListSDO{}, auth: true,
		})
		suite.Post("/tasks/{taskID}/labels", h.AttachTaskLabel, apiOp{
			summary: "Attach label to task", description: "Gắn nhãn vào task.",
		...
		suite.Delete("/tasks/{taskID}/labels/{labelID}", h.DetachTaskLabel, apiOp{
			summary: "Detach label from task", description: "Gỡ nhãn khỏi task.",
			tags: []string{"tasks"}, sdo: sdo.StatusSDO{}, auth: true,
		})
		suite.Put("/tasks/{taskID}/properties/{propertyID}", h.PutTaskPropertyValue, apiOp{
			summary: "Set task property value", description: "Ghi giá trị thuộc tính trên task.",
		...
		suite.Delete("/tasks/{taskID}/properties/{propertyID}", h.DeleteTaskPropertyValue, apiOp{
			summary: "Clear task property value", description: "Xóa giá trị thuộc tính trên task.",
		...
		suite.Get("/workspaces/{workspaceID}/task-views", h.ListTaskViews, apiOp{
			summary: "List task views", description: "Saved views cho một scope. Query: scope_type, scope_id.",
			tags: []string{"tasks"}, sdo: sdo.TaskViewListSDO{}, auth: true,
		})
		suite.Post("/workspaces/{workspaceID}/task-views", h.CreateTaskView, apiOp{
			summary: "Create task view", description: "Tạo saved view.",
		...
		suite.Get("/workspaces/{workspaceID}/task-views/{id}", h.GetTaskView, apiOp{
			summary: "Get task view", description: "Chi tiết một saved view.",
			tags: []string{"tasks"}, sdo: sdo.TaskViewSDO{}, auth: true,
		})
		suite.Patch("/workspaces/{workspaceID}/task-views/{id}", h.PatchTaskView, apiOp{
			summary: "Update task view", description: "Sửa view với expected_revision.",
		...
		suite.Delete("/workspaces/{workspaceID}/task-views/{id}", h.DeleteTaskView, apiOp{
			summary: "Delete task view", description: "Xóa saved view và pin liên quan.",
			tags: []string{"tasks"}, sdo: sdo.StatusSDO{}, auth: true,
		})
		suite.Get("/workspaces/{workspaceID}/task-view-preferences", h.GetTaskViewPreference, apiOp{
			summary: "Get task view preferences", description: "Prefs thanh view theo scope. Query: scope_type, scope_id.",
		...
		suite.Put("/workspaces/{workspaceID}/task-view-preferences", h.PutTaskViewPreference, apiOp{
			summary: "Put task view preferences", description: "Ghi prefs thanh view (last-write-wins).",
		...
		suite.Get("/workspaces/{workspaceID}/pins", h.ListPins, apiOp{
			summary: "List pins", description: "Pin sidebar. Query include=task_view để gồm pin view.",
			tags: []string{"tasks"}, sdo: sdo.TaskPinListSDO{}, auth: true,
		})
		suite.Post("/workspaces/{workspaceID}/pins", h.CreatePin, apiOp{
			summary: "Create pin", description: "Ghim task|project|task_view.",
		...
		suite.Put("/workspaces/{workspaceID}/pins/reorder", h.ReorderPins, apiOp{
			summary: "Reorder pins", description: "Đặt lại position các pin.",
		...
		suite.Delete("/workspaces/{workspaceID}/pins/{itemType}/{itemID}", h.DeletePin, apiOp{
			summary: "Delete pin", description: "Bỏ ghim theo item_type + item_id.",
			tags: []string{"tasks"}, sdo: sdo.StatusSDO{}, auth: true,
		})
		...
		suite.Get("/workspaces/{workspaceID}/projects/search", h.SearchProjects, apiOp{
			summary: "Search projects", description: "Tìm project theo title/description. Query: q, include_closed, limit, offset.",
			tags: []string{"tasks"}, sdo: sdo.ProjectListSDO{}, auth: true,
		})
		suite.Get("/workspaces/{workspaceID}/projects", h.ListProjects, apiOp{
			summary: "List projects", description: "Danh sách project workspace. Query: status, priority.",
			tags: []string{"tasks"}, sdo: sdo.ProjectListSDO{}, auth: true,
		})
		suite.Post("/workspaces/{workspaceID}/projects", h.CreateProject, apiOp{
			summary: "Create project", description: "Tạo project (tuỳ chọn kèm resources).",
		...
		suite.Delete("/workspaces/{workspaceID}/projects/{projectID}", h.DeleteProject, apiOp{
			summary: "Delete project", description: "Xóa project, resources, pin và view theo scope.",
			tags: []string{"tasks"}, sdo: sdo.StatusSDO{}, auth: true,
		})
		suite.Get("/workspaces/{workspaceID}/projects/{projectID}/resources", h.ListProjectResources, apiOp{
			summary: "List project resources", description: "Tài nguyên gắn với project.",
		...
		suite.Post("/workspaces/{workspaceID}/projects/{projectID}/resources", h.CreateProjectResource, apiOp{
			summary: "Create project resource", description: "Thêm github_repo hoặc local_directory.",
		...
		suite.Put("/workspaces/{workspaceID}/projects/{projectID}/resources/{resourceID}", h.PutProjectResource, apiOp{
			summary: "Update project resource", description: "Sửa ref/label/position.",
		...
		suite.Delete("/workspaces/{workspaceID}/projects/{projectID}/resources/{resourceID}", h.DeleteProjectResource, apiOp{
			summary: "Delete project resource", description: "Xóa một resource.",
			tags: []string{"tasks"}, sdo: sdo.StatusSDO{}, auth: true,
		})

		// Collaboration (catalogue group collaboration).
		suite.Put("/comments/{commentID}", h.UpdateComment, apiOp{
			summary: "Update comment", description: "Sửa nội dung comment.",
		...
		suite.Delete("/comments/{commentID}", h.DeleteComment, apiOp{
			summary: "Delete comment", description: "Xóa comment.",
			tags: []string{"tasks"}, sdo: sdo.StatusSDO{}, auth: true,
		})
		suite.Post("/comments/{commentID}/resolve", h.ResolveComment, apiOp{
			summary: "Resolve comment", description: "Đánh dấu comment đã giải quyết.",
			tags: []string{"tasks"}, sdo: sdo.CommentSDO{}, auth: true,
		})
		suite.Delete("/comments/{commentID}/resolve", h.UnresolveComment, apiOp{
			summary: "Unresolve comment", description: "Bỏ đánh dấu giải quyết.",
			tags: []string{"tasks"}, sdo: sdo.CommentSDO{}, auth: true,
		})
		suite.Post("/comments/{commentID}/reactions", h.AddCommentReaction, apiOp{
			summary: "Add comment reaction", description: "Thêm emoji trên comment.",
		...
		suite.Delete("/comments/{commentID}/reactions", h.RemoveCommentReaction, apiOp{
			summary: "Remove comment reaction", description: "Gỡ emoji trên comment.",
		...
		suite.Get("/comments/{commentID}/sub-task-preview", h.CommentSubTaskPreview, apiOp{
			summary: "Sub-task preview", description: "Preview tạo sub-task từ comment (stub).",
			tags: []string{"tasks"}, sdo: sdo.StatusSDO{}, auth: true,
		})
		suite.Post("/comments/{commentID}/sub-tasks", h.CreateCommentSubTasks, apiOp{
			summary: "Create sub-tasks from comment", description: "Tạo sub-task từ comment (stub).",
			tags: []string{"tasks"}, sdo: sdo.StatusSDO{}, auth: true,
		})
		suite.Post("/tasks/{taskID}/reactions", h.AddTaskReaction, apiOp{
			summary: "Add task reaction", description: "Thêm emoji trên task.",
		...
		suite.Delete("/tasks/{taskID}/reactions", h.RemoveTaskReaction, apiOp{
			summary: "Remove task reaction", description: "Gỡ emoji trên task.",
			tags: []string{"tasks"}, sdi: sdi.ReactionSDI{}, sdo: sdo.StatusSDO{}, auth: true,
		})
		suite.Get("/tasks/{taskID}/subscribers", h.ListTaskSubscribers, apiOp{
			summary: "List task subscribers", description: "Danh sách người theo dõi task.",
			tags: []string{"tasks"}, sdo: sdo.TaskSubscriberListSDO{}, auth: true,
		})
		suite.Post("/tasks/{taskID}/subscribe", h.SubscribeTask, apiOp{
			summary: "Subscribe to task", description: "Theo dõi task.",
		...
		suite.Post("/tasks/{taskID}/unsubscribe", h.UnsubscribeTask, apiOp{
			summary: "Unsubscribe from task", description: "Bỏ theo dõi task.",
			tags: []string{"tasks"}, sdi: sdi.SubscribeTaskSDI{}, sdo: sdo.StatusSDO{}, auth: true,
		})
		suite.Post("/tasks/{taskID}/unsubscribe/subtree", h.UnsubscribeTaskSubtree, apiOp{
			summary: "Unsubscribe subtree", description: "Bỏ theo dõi task và mọi con.",
			tags: []string{"tasks"}, sdi: sdi.SubscribeTaskSDI{}, sdo: sdo.StatusSDO{}, auth: true,
		})
		suite.Get("/tasks/{taskID}/attachments", h.ListTaskAttachments, apiOp{
			summary: "List task attachments", description: "Danh sách đính kèm trên task.",
		...
		suite.Get("/attachments/{attachmentID}/content", h.GetAttachmentContent, apiOp{
			summary: "Preview attachment content", description: "Stream nội dung để preview (CSP frame-ancestors self).",
			tags: []string{"tasks"}, auth: true,
		})
		suite.Get("/attachments/{attachmentID}/download", h.DownloadAttachment, apiOp{
			summary: "Download attachment", description: "Stream tải về với Content-Disposition attachment.",
			tags: []string{"tasks"}, auth: true,
		})
