...
// with fields.reason_code aligned to workcapability catalogue keys.
func registerTasksSuiteStubs(suite api, h Routes) {
	...
	suite.Get("/tasks/{taskID}/active-task", h.WorkManagementCapabilityStub, op(
		...
		"Returns 422 capability_unavailable with reason_code agent_runtime_missing. Flag tasks_work_management_parity.",
	))
	...
	suite.Get("/tasks/{taskID}/pull-requests", h.WorkManagementCapabilityStub, op(
		...
		"Returns 422 capability_unavailable with reason_code vcs_provider_missing. Flag tasks_work_management_parity.",
	))
	...
	suite.Post("/tasks/{taskID}/terminate", h.WorkManagementCapabilityStub, op(
		...
		"Alias of cancel. Returns 422 capability_unavailable with reason_code agent_runtime_missing. Flag tasks_work_management_parity.",
	))
	...
	suite.Get("/workspaces/{workspaceID}/squads", h.WorkManagementCapabilityStub, op(
		"List squads (stub)",
		"Returns 422 capability_unavailable with reason_code squad_directory_missing. Flag tasks_work_management_parity.",
	))
	...
	suite.Get("/workspaces/{workspaceID}/workdir", h.WorkManagementCapabilityStub, op(
		"Workdir status (stub)",
		"Returns 422 capability_unavailable with reason_code local_daemon_missing. Flag tasks_work_management_parity.",
	))
