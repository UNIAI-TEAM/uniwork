...
| F-14 | i18n vi/en parity gate trong CI, ngôn ngữ my/km/lo gắn beta | Cross-cutting | CÓ (2026-09-15, UNI-436; chưa có bản dịch my/km/lo nào; locale thêm sau tự mang nhãn Beta ở mọi bộ chọn, các bước ở conventions §2) | Gate `parity.test.ts` cho mọi file locale (khóa, cặp số nhiều, biến nội suy, không lẫn tiếng Việt) + `i18n-duplicate-keys` trong CI; `STABLE_LOCALES` và badge Beta ở mọi bộ chọn, locale beta không tự chọn theo trình duyệt; lint giọng văn vi `voice.test.ts` | `docs/conventions.md` §2–§3 | P2 |
