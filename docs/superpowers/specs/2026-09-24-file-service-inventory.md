...
| 8 | `packages/views/chat/use-chat-file-object-url.ts:11-40`, `packages/views/chat/chat-file-attachment.tsx:79-84` | File/voice chat: tải bytes qua `/file` (tin nhắn file, `router/chat.go:265`) và `/voice` (tin nhắn thoại, `router/chat.go:251`) — react-query theo message, dựng `URL.createObjectURL`, thu hồi khi unmount/thay | URL là blob của phiên render, không có hạn; bytes nằm trong query cache |
...
| U745-4 | Tải/stream: `GET …/messages/{id}/file` (`router/chat.go:265`) kiểm quyền phòng rồi stream từ storage; ảnh inline, file khác attachment (`chat_file_message.go:161-169`) | `handler/chat_file_message_test.go:91 TestSendAndStreamChatFileMessage` (phủ quyền + bytes + DTO; **test chưa phủ** phần `Content-Disposition`/`Cache-Control`) |
...
| U745-6 | Phát voice: FE tải blob qua `/voice` (`router/chat.go:251`, `chat-messages.ts:161-168`), cache theo message, dựng/thu hồi object URL theo vòng đời mount | `views/chat/use-chat-file-object-url.ts`; `core/chat/voice-playback-store.test.ts`; `views/chat/use-chat-media-send.test.tsx` |
...
| 9 | Tester (vòng 2) | §5 hàng 8, U745-4 và U745-6 nói bytes tin nhắn chat đi qua `/content`; route thật là `/file` (`router/chat.go:265`, `chat-messages.ts:197-204`) và `/voice` (`router/chat.go:251`, `chat-messages.ts:161-168`); `/content` chỉ đúng cho recording (`router/chat.go:392`) và attachment (`/api/v1/attachments/...`) | sửa cả ba chỗ; giữ nguyên các câu `/content` của attachment và recording |
