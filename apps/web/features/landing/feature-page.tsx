...
import { featureCopyPrefix, type FeaturePageKey } from "./feature-page-catalog";
...
export function FeaturePage({ featureKey }: { featureKey: FeaturePageKey }) {
  ...
  const prefix = featureCopyPrefix(featureKey);
  ...
  return <MarketingShell>
    <section className="feature-page-hero">
      <div className="feature-page-copy">
        ...
        <div className="marketing-actions"><Link href={paths.register()} className={buttonVariants({ variant: "brand", size: "lg" })}>{t("landing.cta.start")}<ArrowRight aria-hidden /></Link><Link href={`/#${feature.anchor}`} className={buttonVariants({ variant: "outline", size: "lg" })}><Play aria-hidden />{t(hasPlayback(featureKey) ? "landing.productPages.watchDemo" : "landing.revision.viewReference")}</Link></div>
    ...
    <FeatureContext feature={featureKey} />
