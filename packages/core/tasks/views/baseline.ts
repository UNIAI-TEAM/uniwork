 ...
 */
export interface TaskViewBaseline {
  status: Set<string>;
  priority: Set<string>;
...
export function baselineFromQuery(query: Record<string, unknown>): TaskViewBaseline {
  ...
  //
  // Status is the exception: a status filter holds a status KEY, and a
  // workspace's custom keys are not enumerable from a constant. Filtering
  // against a fixed enum here silently deleted every custom status filter the
  ...
  const statusFilters = stringArray(query.statusFilters).filter(
    (s): s is TaskStatusKey => s.length > 0,
  );
  const priorityFilters = stringArray(query.priorityFilters).filter(
    (p): p is TaskPriority => (TASK_PRIORITIES as readonly string[]).includes(p),
  );
