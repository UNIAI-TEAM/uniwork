...
/** Task count is opt-in; the rest show by default (matching the prior
 *  compact table). */
export const PROJECT_DEFAULT_HIDDEN_COLUMNS: ProjectColumnKey[] = ["tasks"];
...
export function resetProjectViewStoreForTests(): void {
  useProjectViewStore.setState({ ...DEFAULTS });
}
