 ...
 * markdown bodies (task descriptions, comment bodies, chat messages).
 *
 * Background — UNI-3130:
 *
 *   The original upload flow returned `att.url` as the link to embed in
 *   markdown. After PR #3903 (UNI-3132), `att.url` for the LocalStorage
 ...
 *   attachment id. The server's /api/v1/attachments/{id}/download endpoint
...
const DOWNLOAD_PREFIX = "/api/v1/attachments/";
const DOWNLOAD_SUFFIX = "/download";
const CHANNEL_MEDIA_MARKER_RE =
  /<!-- uniwork:channel-media:[0-9a-fA-F-]{36} -->/g;
 ...
 * `/api/v1/attachments/` and `/download` so we never accept a non-uuid
 * path component as an id.
 */
const ID_RE = /^(?:[0-9A-HJKMNP-TV-Z]{26}|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/i;
...
/**
 * Extract the attachment id from a `/api/v1/attachments/<uuid>/download`
 * URL (with or without query/host). Returns `undefined` if the URL is
 * not a download URL, or if the captured segment is not a valid attachment id
 * literal.
 *
 ...
 * either the new stable `/api/v1/attachments/<id>/download` shape OR the
 ...
 *   - new uploads from a post-UNI-3130 client write the stable URL
 *   - edits to a pre-UNI-3130 comment may still have the legacy URL
 *     in `entry.content`
