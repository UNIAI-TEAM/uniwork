...
function getSessionStorage(): Storage | null {
  try {
    // Reached directly rather than through StorageAdapter: dedupe runs on the
    // crash path, which can fire before the platform layer has been wired, and
    // an exception reporter that depends on boot order reports nothing exactly
    // when it matters. The absence and throw cases are both handled below,
    // which is what the adapter rule is protecting against.
