...
const WORKSPACE_SECTIONS = new Set(
  WORKSPACE_ROUTES.filter((route) => route.length > 0).map((route) => route[0]!),
);
...
export function bucketDiagnosticPath(path: string): string {
  ...
  // Everything else is workspace-scoped, and workspaces nest inside an
  // organization: the first two segments are /{orgSlug}/{workspaceSlug}.
  if (segments.length === 1) return "/:org";
  const rest = segments.slice(2);

  const scoped = matchRoute(rest, WORKSPACE_ROUTES);
  if (scoped) {
    return scoped.length === 0 ? "/:org/:slug" : `/:org/:slug/${scoped.join("/")}`;
  }
