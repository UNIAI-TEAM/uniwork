...
function builderTemplates(): string[] {
  ...
  const scoped = [ws.root(), ws.tasks(), ws.task(ID), ws.myTasks(), ws.projects(), ws.project(ID), ws.meetings(), ws.meeting(ID), ws.room(ID), ws.chat(), ws.inbox(), ws.email(), ws.members(), ws.people(), ws.person(ID), ws.settings()];
