...
describe("board-drag-utils", () => {
  const groups: BoardColumnGroup[] = [
    {
      id: statusGroupId("todo"),
      title: "Todo",
      kind: "status",
      status: "todo",
    },
    {
      id: statusGroupId("done"),
      title: "Done",
      kind: "status",
      status: "done",
    },
  ...
  it("builds assignee and project columns", () => {
    const assigneeGroups: BoardColumnGroup[] = [
      {
        id: assigneeGroupId("human", "u1"),
        title: "An",
        kind: "assignee",
        assigneeId: "u1",
        assigneeKind: "human",
    ...
    expect(
      buildColumns(
        [task({ id: "a", assignee_id: "u1" }), task({ id: "b" })],
        assigneeGroups,
      ),
    ).toEqual({
      [assigneeGroupId("human", "u1")]: ["a"],
      [assigneeGroupId("human", null)]: ["b"],
    });

    const projectGroups: BoardColumnGroup[] = [
      {
        id: projectGroupId("p1"),
        title: "Alpha",
        kind: "project",
        projectId: "p1",
      },
    ...
    expect(
      buildColumns(
        [task({ id: "a", project_id: "p1" }), task({ id: "b" })],
        projectGroups,
      ),
    ).toEqual({
      [projectGroupId("p1")]: ["a"],
  ...
  it("inserts ids by position", () => {
    ...
    expect(insertIdByPosition(["a", "c"], "b", 3, map)).toEqual([
      "a",
      "b",
      "c",
    ]);
    expect(insertIdByPosition(["a", "c"], "z", 9, map)).toEqual([
      "a",
      "c",
      "z",
    ]);
  ...
  it("finds columns by id or contained task", () => {
    ...
    expect(findColumn(columns, statusGroupId("todo"), ids)).toBe(
      statusGroupId("todo"),
    );
  ...
  it("matches groups and builds move updates", () => {
    const todo = groups[0]!;
    const assignee: BoardColumnGroup = {
      id: assigneeGroupId("human", "u2"),
      title: "Binh",
      kind: "assignee",
      assigneeId: "u2",
      assigneeKind: "human",
    ...
    expect(taskMatchesGroup(task({ assignee_id: "u2" }), assignee)).toBe(true);
    expect(getMoveUpdates(todo, 2, task({ status: "todo" }))).toEqual({
      position: 2,
    });
    ...
    expect(getMoveUpdates(assignee, 4, task())).toEqual({
      assignee_id: "u2",
      assignee_kind: "human",
      position: 4,
    });
