...
import {
  ...
  type TaskIdentifierResolver,
} from "./task-identifier-autolink";
...
export interface EditorExtensionsOptions {
   ...
   * another UniWork editor renders as the normal mention pill instead of
   ...
   * turns a pick into the text it would post (UNI-0). Both are functions so
...
export function createEditorExtensions(
  ...
  return [
    StarterKit.configure({
      ...
      // the editor draw every H4–H6 as an H1 (UNI-0). The same list drives
    ...
    // typed this trigger" signal their `shouldShow` reads (UNI-0).
