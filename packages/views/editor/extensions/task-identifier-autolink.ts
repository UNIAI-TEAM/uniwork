/**
 * TaskIdentifierAutolink — Linear-style live autolinking of bare task
 * identifiers (e.g. `UNI-0`) in the editable editor.
 *
 ...
 * replaced with a real `task` mention node. On save the mention serialises to
 * the canonical `[UNI-0](mention://task/<uuid>)`.
 *
...
export interface ResolvedTaskRef {
  /** Task UUID. */
  id: string;
  /** Canonical identifier as returned by the server, e.g. "UNI-0". */
  identifier: string;
}

export type TaskIdentifierResolver = (
  identifier: string,
) => Promise<ResolvedTaskRef | null>;

export interface TaskIdentifierAutolinkOptions {
  /**
   ...
   */
  resolveRef: RefObject<TaskIdentifierResolver | undefined>;
}

// Boundary-delimited, case-sensitive identifier — same shape as the readonly
// detector in @uniwork/ui/markdown.
...
export function createTaskIdentifierAutolinkExtension(
  options: TaskIdentifierAutolinkOptions,
): Extension {
      ...
      return [
        new Plugin<AutolinkPluginState>({
          ...
          view(view) {
            ...
            const resultCache = new Map<string, ResolvedTaskRef | null>();
