...
export function settleUploadNode(editor: any, uploadId: string, result: UploadResult): boolean {
  ...
  // (UNI-0). `link` is the fallback for the no-workspace avatar branch.
...
export async function uploadAndInsertFile(
  ...
  if (isImage) {
    ...
    try {
      ...
      // The upload outlives the mount (coordinator-owned, UNI-0): by the
          ...
          props: {
            ...
            handleDrop(view, event) {
              ...
              for (let i = 0; i < unique.length; i++) {
                ...
                void uploadAndInsertFile(editor, unique[i]!, handler, insertPos);
              }
              return true;
            },
          },
