...
interface SlashCommandListProps {
   ...
   * matching items. Used by the built-in command menu in task comments, where
...
>(function SlashCommandList({ items, query, command, hideOnEmpty = false }, ref) {
  ...
  if (items.length === 0) {
    ...
    return (
      <div className="rounded-md border bg-popover p-2 text-caption text-muted-foreground shadow-md">
        {t(
          query.trim()
            ? "editor.slash_command.no_results"
            : "editor.slash_command.no_skills_configured",
        )}
      </div>
    );
  }
  ...
  const describe = (item: SlashCommandItem): string | undefined =>
    item.descriptionKey === "note"
      ? t("editor.slash_command.commands.note")
      : item.description;
...
/** Chat skill slash list — UniWork has no uniwork agent-skills picker yet. */
function buildItems(_qc: QueryClient, _query: string): SlashCommandItem[] {
  return [];
}
...
> {
  ...
  return {
    ...
    // (`/usr/local/bin`) never opens the skill picker (UNI-0).
...
// ---------------------------------------------------------------------------
// Built-in command menu (task comments)
// ---------------------------------------------------------------------------

/**
 * Built-in slash commands offered in the task comment composer. Unlike the
...
export function buildBuiltinCommandItems(
  ...
  // Quick actions lead: on an task they are the reason a user reaches for
...
export function createBuiltinCommandSuggestion(
  ...
  return {
    ...
    pluginKey,
    // Only open over a `/` the user actually typed, so a pasted path
    // (`/usr/local/bin`) never opens the command menu (UNI-0).
    shouldShow: ({ editor, range }) => isTriggerArmedAt(editor, range.from),
