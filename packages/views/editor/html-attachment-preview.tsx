...
export function HtmlAttachmentPreview({
  ...
  const isError = !query.isLoading && (!!query.error || !query.data);
  // useEditorWorkspaceSlug — NOT useWorkspacePaths. The Paths-bound variant throws
  ...
  const handleOpenInNewTab = () => {
    ...
    const path = `/api/v1/attachments/${attachmentId}/content${nameQuery}`;
    const url = navigation?.getShareableUrl?.(path) ?? path;
    window.open(url, "_blank", "noopener,noreferrer");
  };

  return (
    // eslint-disable-next-line jsx-a11y/no-static-element-interactions -- mousedown stopPropagation; toolbar uses buttons
    <div
