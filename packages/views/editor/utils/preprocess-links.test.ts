...
import { preprocessLinks } from "@uniwork/ui/markdown/linkify";
...
describe("preprocessLinks — CJK punctuation boundary", () => {
  ...
  it("handles the original bug report (PR link then 。 then more text)", () => {
    const out = preprocessLinks(
      "已合并 PR #1623：https://github.com/uniwork-ai/uniwork/pull/1623。merge commit",
    );
    expect(out).toBe(
      "已合并 PR #1623：[https://github.com/uniwork-ai/uniwork/pull/1623](https://github.com/uniwork-ai/uniwork/pull/1623)。merge commit",
    );
  });
...
// swallow was the UNI-0 render bug. Mirrors GFM's own autolink trailing trim.
describe("preprocessLinks — trailing markdown delimiter is not part of the URL", () => {
  ...
  it("keeps ** outside when a CJK punctuation immediately follows (variant B)", () => {
    expect(preprocessLinks("**https://example.com/x**（UNI-0）")).toBe(
      "**[https://example.com/x](https://example.com/x)**（UNI-0）",
    );
  });
