...
import { stripChannelMediaMarkers } from "@uniwork/core/types";
 ...
 * 2. (readonly only) Bare issue identifiers UNI-0 → [UNI-0](mention://task/UNI-0)
 ...
 * `editorConfigStore.getState()` read inside this function. The CDN config arrives
...
export function preprocessMarkdown(
  ...
  const step2 = opts?.autolinkIssueIdentifiers
    ? preprocessTaskIdentifiers(step1)
    : step1;
