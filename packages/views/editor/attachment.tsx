 ...
 *     serving Chat, Task descriptions and Comments through one renderer)
...
function normalize(
  ...
  return {
    ...
    // markdown stays on its existing path. See UNI-0 review.
    //
    ...
    // See UNI-0 — quick-create modal regressed because the freshly-
...
// `/uploads/<key>` URL.
//
// The current (post-UNI-0) write path persists an absolute URL chosen
...
// matters for content written BEFORE UNI-0 — those bodies still carry
...
//                             persistence (UNI-0 / `buildMarkdownURL`):
//                             public CDN passthrough when the storage is
//                             public-readable, or `uniwork_PUBLIC_URL +
...
//     bucket (must-fix 2 from UNI-0 review), except for the explicit
//     site-relative local upload path above.
//  5. `record.url` — legacy fallback for responses that omit
//     `markdown_url` (a backend old enough to predate UNI-0).
...
function ImageAttachmentView({
  ...
  const handleCopyLink = async () => {
    if (await copyText(linkUrl)) {
      toast.success(t("editor.image.link_copied"));
    } else {
      toast.error(t("editor.image.copy_link_failed"));
    }
  };
  ...
  return (
    <span className="image-node">
      {/* eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions -- span-only DOM stays valid in markdown <p>; Maximize button is the accessible open */}
      <span
        className={cn(
          "image-figure",
      ...
      >
        ...
        />
        {!uploading && src && (
          // eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions -- toolbar stops propagation; children are buttons
          <span
