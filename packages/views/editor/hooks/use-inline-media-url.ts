...
/**
 * Inline media re-sign (UNI-0).
 *
 * Extracted from editor/attachment.tsx so the preview modal can run the same
 * upgrade: gallery navigation (UNI-0) hands the modal an attachment the
...
import { attachmentIdFromDownloadURL } from "@uniwork/core/types/attachment-url";
...
// auth-gated API path again, and before UNI-0 the renderer simply kept the
...
export function useResignedInlineMediaURL(
  ...
  const isCrossOriginWebURL = (() => {
    if (!/^https?:\/\//i.test(pickedUrl)) {
      return false;
    }
    try {
      return new URL(pickedUrl).origin !== new URL(runtimeConfig().appUrl).origin;
    } catch {
      return false;
    }
  })();
