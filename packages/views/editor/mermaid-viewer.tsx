  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        aria-label={t("editor.mermaid.viewer_title")}
      >
      ...
      <header className="flex shrink-0 items-center gap-1 border-b border-border bg-muted/30 px-3 py-2">
          ...
          <div className="ml-auto flex items-center gap-0.5">
            <ToolbarButton
              ...
              label={
                showSource
                  ? t("editor.mermaid.show_diagram")
                  : t("editor.mermaid.show_source")
              }
            >
            ...
            <ToolbarButton
              ...
              label={t("editor.mermaid.copy_source")}
            >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger
                render={
                  <button
                    type="button"
                    title={t("editor.mermaid.export")}
                    aria-label={t("editor.mermaid.export")}
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={handleDownloadPng} disabled={!canExportImage}>
                  ...
                  {t("editor.mermaid.export_png")}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={handleDownloadSvg} disabled={!canExportImage}>
                  <ImageIcon className="size-4" />
                  {t("editor.mermaid.export_svg")}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={handleDownloadSource}>
                  <CodeIcon className="size-4" />
                  {t("editor.mermaid.export_source")}
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            <ToolbarButton
              onClick={onClose}
              label={t("editor.mermaid.close_viewer")}
            >
              <X className="size-4" />
            </ToolbarButton>
          </div>
        ...
        {showSource ? (
          ...
          <ZoomCanvas
            canvas={canvas}
            content={layout}
            label={t("editor.mermaid.canvas_label")}
            className="bg-muted"
            canvasRef={canvasRef}
          >
            {viewerDocument && (
              <iframe
                ...
                title={t("editor.mermaid.viewer_title")}
