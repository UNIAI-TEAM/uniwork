...
import { matchAttachmentByURL } from "@uniwork/core/attachments/image-sequence";
 ...
 * (post-UNI-0) persist the stable `/api/attachments/<id>/download`
 ...
 * triggered UNI-0. The id-from-URL extractor handles new content;
...
export function AttachmentDownloadProvider({ attachments, children }: ProviderProps) {
  ...
  const value = useMemo<ResolvedDownload>(
    () => {
      // Shared with the gallery sequence builder (UNI-0) so a markdown URL
