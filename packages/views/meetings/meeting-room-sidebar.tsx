...
export function MeetingRoomSidebar({
  ...
  return (
    <aside className={cn("flex min-h-0 min-w-0 w-full flex-col bg-surface", className)}>
      <Tabs defaultValue="participants" className="flex min-h-0 w-full flex-1 flex-col gap-0 p-3">
        ...
        <TabsContent value="chat" className="mt-3 flex min-h-0 w-full flex-1 flex-col">
          <div className="min-h-0 w-full flex-1 space-y-4 overflow-y-auto">
            {groups.length === 0 ? (
              <p className="text-caption text-muted-foreground">{t("meetings.chatEmpty")}</p>
            ) : (
              groups.map((group) => (
                <div
                  key={`${group.fromIdentity}-${group.items[0]?.id ?? ""}`}
                  className={cn("w-full space-y-1.5", group.isLocal && "flex flex-col items-end")}
                >
                  <div className={cn("flex min-w-0 items-center gap-1.5", group.isLocal && "flex-row-reverse")}>
                    {group.isLocal ? (
                      <span className="rounded-full border border-brand/28 bg-brand/7 px-1.5 py-px text-caption font-medium text-foreground">
                        {t("meetings.you")}
                      </span>
                    ) : null}
                    <span className="min-w-0 truncate text-caption font-medium text-foreground">{group.fromName}</span>
