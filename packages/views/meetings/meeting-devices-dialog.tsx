    ...
    <div className="space-y-6">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]">
        <section className="space-y-2">
          ...
          <MeetingCameraPreview deviceId={cameras.activeDeviceId} active />
        </section>
          ...
          <div className="space-y-4 border-t border-border pt-4">
            <label className="flex min-h-11 items-start justify-between gap-3">
              <span className="min-w-0 space-y-0.5">
                <span className="block text-body text-foreground">{t("meetings.deviceMirror")}</span>
                <span className="block text-caption text-muted-foreground">
                  {t("meetings.deviceMirrorHint")}
                </span>
              </span>
              <Switch className="mt-0.5 shrink-0" checked={mirrorCamera} onCheckedChange={setMirrorCamera} />
            </label>
            <label className="flex min-h-11 items-start justify-between gap-3">
              <span className="min-w-0 space-y-0.5">
                <span className="block text-body text-foreground">{t("meetings.deviceExpandedLabels")}</span>
                <span className="block text-caption text-muted-foreground">
                  {t("meetings.deviceExpandedLabelsHint")}
                </span>
              </span>
              <Switch
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-h-[min(92dvh,44rem)] overflow-y-auto sm:max-w-3xl">
        ...
        {open ? <MeetingDevicesPanel onReload={() => setRefreshKey((value) => value + 1)} key={refreshKey} /> : null}
      </DialogContent>
    </Dialog>
  );
}
