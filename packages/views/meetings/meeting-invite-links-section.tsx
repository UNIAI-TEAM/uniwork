        ...
        <ul className="mb-3 divide-y divide-border rounded-lg border border-border bg-surface">
          {(links ?? []).map((link) => {
            ...
            return (
              <li
                key={link.id}
                className="flex min-w-0 flex-wrap items-center justify-between gap-2 px-3 py-2.5"
              >
                <div className="min-w-0">
                  ...
                  <div className="text-caption text-muted-foreground">
                    {link.access_mode === "REQUEST_APPROVAL"
                      ? t("meetings.linkNeedApproval")
                      : t("meetings.linkAutoAdmit")}
                  </div>
                </div>
        ...
        >
          <FieldGroup>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <Field>
                <FieldLabel htmlFor="link-days">
                  {t("meetings.linkExpiry")}
                </FieldLabel>
                <Select
                  id="link-days"
                  value={days}
                  ...
                  items={["1", "7", "30"].map((d) => ({
                    value: d,
                    label: t("meetings.linkDays", { count: Number(d) }),
                  }))}
                />
              </Field>
              <Field>
                <FieldLabel htmlFor="link-access">
                  {t("meetings.linkAccess")}
                </FieldLabel>
                <Select
                  id="link-access"
                  value={mode}
                  ...
                  items={[
                    { value: "AUTO_ADMIT", label: t("meetings.linkAutoAdmit") },
                    {
                      ...
                      label: t("meetings.linkNeedApproval"),
                    },
                  ]}
                />
              </Field>
            ...
            <Field>
              <FieldLabel htmlFor="link-max">
                {t("meetings.linkMaxUses")}
              </FieldLabel>
              <Input
                id="link-max"
                type="number"
