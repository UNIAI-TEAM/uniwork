                  ...
                  <section className="flex flex-col gap-2">
                    ...
                    <ul className="space-y-2">
                      {result.action_items.map((item) => {
                        const canCreate = Boolean(item.source_message_id);
                        return (
                          <li key={`${item.title}-${item.source_message_id}`}>
                            {canCreate ? (
                              <button
                                type="button"
                                className="w-full rounded-md border border-border bg-surface px-3 py-2.5 text-left transition-colors hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
